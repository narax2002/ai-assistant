...
This repository is still in the bootstrap phase. `README.md` provides the high-level summary, and design notes now live under `markdown/`, currently `markdown/ai-assistant.md`. As implementation is added, keep the planned Python layout:
...
- `markdown/` for planning notes and contributor-facing docs
...
Target Python 3.11. Use 4-space indentation, `snake_case` for modules and functions, `PascalCase` for classes, and `UPPER_SNAKE_CASE` for environment variables. Keep provider-specific code inside `llm/` and configuration in `.env`, not hardcoded paths. Store long-form notes in `markdown/` using descriptive lowercase file names such as `deployment-plan.md`.
